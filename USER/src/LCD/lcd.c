 ...
 * @retval      无
 */
void lcd_init(FSMC_NORSRAM_TimingTypeDef *Timing,FSMC_NORSRAM_TimingTypeDef *ExtTiming)
{
//    GPIO_InitTypeDef gpio_init_struct;
    FSMC_NORSRAM_TimingTypeDef fsmc_read_handle;
    FSMC_NORSRAM_TimingTypeDef fsmc_write_handle;
	
	  fsmc_read_handle=*Timing;
    fsmc_write_handle=*ExtTiming;
...
//    HAL_SRAM_Init(&g_sram_handle, &fsmc_read_handle, &fsmc_write_handle);
    ...
    else if (lcddev.id == 0x9341 || lcddev.id == 0x1963)  /* 如果是这几个IC, 则设置WR时序为最快 */
    {
        /* 重新配置写时序控制寄存器的时序 */
    ...
    else if (lcddev.id == 0x7789)  /* 如果是这个IC, 则设置WR时序为最快 */
    {
        /* 重新配置写时序控制寄存器的时序 */
        fsmc_write_handle.AddressSetupTime = 4;
        fsmc_write_handle.DataSetupTime = 3;         
        FSMC_NORSRAM_Extended_Timing_Init(g_sram_handle.Extended, &fsmc_write_handle, g_sram_handle.Init.NSBank, g_sram_handle.Init.ExtendedMode);
    }
