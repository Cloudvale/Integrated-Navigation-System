...
//    
    ...
    {
        rtc_set_time(13, 17, 56, RTC_HOURFORMAT12_AM);   /* 设置时间, 根据实际时间修改 */
        rtc_set_date(23, 7, 18, 2);                     /* 设置日期 */
    }
    return 0;
}
           ...
           if (key == KEY2_PRES)
					 {
						 ...
						 case 1:
							 min++;
						  if(min==60) min=0;
					    rtc_set_time(hour, min, sec, RTC_HOURFORMAT12_AM);   /* 设置时间, 根据实际时间修改 */
			...
			sprintf((char *)tsbuf, "Date:20%02d-%02d-%02d,Time:%02d:%02d:%02d\n", year, month, date, hour, min, sec);
			lcd_show_string(10, 290, 400, 30, 24, (char*)tsbuf, BLACK);
    }
}
