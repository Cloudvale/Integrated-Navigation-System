...
#define LWIP_DEMO_RX_BUFSIZE         1000  /* 定义udp最大接收数据长度 */
#define LWIP_DEMO_PORT               8080  /* 定义udp连接的本地端口号 */

/* 接收数据缓冲区 */
uint8_t g_lwip_demo_recvbuf[LWIP_DEMO_RX_BUFSIZE]; 
/* 发送数据内容 */
//char *g_lwip_demo_sendbuf = "ALIENTEK DATA1111111111111111111111111111111111111111111111111\r\n";
/* 数据发送标志位 */
...
//    tbuf = mymalloc(SRAMIN, 200); /* 申请内存 */
    ...
    if (sizeof(ftbuf) == NULL)return ;     /* 内存申请失败了,直接退出 */
//    if (tbuf == NULL)return ;     /* 内存申请失败了,直接退出 */

    sprintf((char *)ftbuf, "Local IP:%d.%d.%d.%d", g_lwipdev.ip[0], g_lwipdev.ip[1], g_lwipdev.ip[2], g_lwipdev.ip[3]); /* 服务器IP */
//    lcd_show_string(5, 130, 210, 16, 16, tbuf, g_point_color);
    sprintf((char *)ftbuf, "Remote IP:%d.%d.%d.%d", g_lwipdev.remoteip[0], g_lwipdev.remoteip[1], g_lwipdev.remoteip[2], g_lwipdev.remoteip[3]); /* 远端IP */
//    lcd_show_string(5, 150, 210, 16, 16, tbuf, g_point_color);
    sprintf((char *)ftbuf, "Remote Port:%d", LWIP_DEMO_PORT); /* 客户端端口号 */
        ...
        if (err == ERR_OK)
        {
            ...
            if (err == ERR_OK)  /* 绑定完成 */
            {
                udp_recv(udppcb,lwip_udp_callback, NULL);                                                             /* 注册接收回调函数 */
									...
									lcd_show_string(10+12*9,130,50,30,24," OK",BLUE);                           	                    	/* “ok”显示，减轻刷新频率 */
//                g_point_color = BLUE;
...
//	
//	
//    ptr = pbuf_alloc(PBUF_TRANSPORT, strlen((char *)sd_send_gnss), PBUF_POOL); /* 申请内存 */
//    ptrs = pbuf_alloc(PBUF_TRANSPORT, strlen((char *)sd_send_imu), PBUF_POOL); /* 申请内存 */

//	  
//    if (ptr && ptrs)
//    {
//				pbuf_take(ptr, (char *)sd_send_gnss, strlen((char *)sd_send_gnss));
//				pbuf_take(ptrs, (char *)sd_send_imu, strlen((char *)sd_send_imu));
//				udp_send(upcb, ptr);    /* udp发送数据 */
//				udp_send(upcb, ptrs);    /* udp发送数据 */
//				pbuf_free(ptr);         /* 释放内存 */
//				pbuf_free(ptrs);         /* 释放内存 */
//    }
		struct pbuf *udp_sends,*udp_sends_2;
    num_assign();
	
//	 udp_sends = pbuf_alloc(PBUF_TRANSPORT, strlen(send_udp_str), PBUF_POOL);
	 udp_sends_2 = pbuf_alloc(PBUF_TRANSPORT, strlen(send_udp_str_2), PBUF_POOL);
	
	if(udp_sends_2)
	 {
//		 pbuf_take(udp_sends, (char *)send_udp_str, strlen((char *)send_udp_str));
//		 uint8_t err_ok = udp_send(upcb, udp_sends);    /* udp发送数据 */
//		 pbuf_free(udp_sends);         /* 释放内存 */
		 
	  pbuf_take(udp_sends_2, (char *)send_udp_str_2, strlen((char *)send_udp_str_2));
		udp_send(upcb, udp_sends_2);    /* udp发送数据 */
		 pbuf_free(udp_sends_2);         /* 释放内存 */
	 }
