...
//				info->accel.z = get_signed_int(data + SINGLE_DATA_BYTES * 2) * NOT_MAG_DATA_FACTOR;
				info->accel.z = -get_signed_int(data + SINGLE_DATA_BYTES) * NOT_MAG_DATA_FACTOR;
				info->accel.y = get_signed_int(data + SINGLE_DATA_BYTES * 2) * NOT_MAG_DATA_FACTOR;
...
//				info->angle_rate.z = get_signed_int(data + ANGLE_DATA_LEN - SINGLE_DATA_BYTES) * NOT_MAG_DATA_FACTOR;
				...
				info->angle_rate.z = -get_signed_int(data + SINGLE_DATA_BYTES) * NOT_MAG_DATA_FACTOR;
				info->angle_rate.y = get_signed_int(data + ANGLE_DATA_LEN - SINGLE_DATA_BYTES) * NOT_MAG_DATA_FACTOR;
			}
			else
			{
				ret = data_len_err;
...
int analysis_data(unsigned char *data, short len, protocol_info_t *info)
{
   ...
   headers.header1=header->header1;
	 headers.header2=header->header2;
	 headers.tid=header->tid;
