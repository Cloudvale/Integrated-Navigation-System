    ...
    while(GPNON != (ptype = nmea_parser_pop(parser, &pack)))
    {
        ...
        case GPVTG:
            ...
            break;
				...
				/*add-function*/
				    case GLGSV:
            nmea_GPGSV2info((nmeaGLGSV *)pack, info);
            break;
						case GAGSV:
            nmea_GPGSV2info((nmeaGAGSV *)pack, info);
            break;
						case GQGSV:
            nmea_GPGSV2info((nmeaGQGSV *)pack, info);
            break;
						case KSXT:
            nmea_KSXT2info((nmeaKSXT *)pack, info);
            break;
						case PSRDOPA:
            nmea_PSRDOPA2info((nmeaPSRDOPA *)pack, info);
            break;
						case GPHDT:
        ...
        else if(crc >= 0)
        {
            ...
            case GPVTG:
                ...
                break;
							...
							/*add the function*/		  /*GLGSV、GAGSV、GQGSV三者结构相似，采用相同的解法*/          
						 case GLGSV:
                if(0 == (node->pack = malloc(sizeof(nmeaGLGSV))))
                    goto mem_fail;
                node->packType = GLGSV;
								//FIX-NOW
                if(!nmea_parse_GLGSV(
                    (const char *)parser->buffer + nparsed,
                    sen_sz, (nmeaGLGSV *)node->pack))
                {
                    free(node);
                    node = 0;
                }
                break;
								case GAGSV:
                if(0 == (node->pack = malloc(sizeof(nmeaGAGSV))))
                    goto mem_fail;
                node->packType = GAGSV;
								//FIX-NOW
                if(!nmea_parse_GAGSV(
                    (const char *)parser->buffer + nparsed,
                    sen_sz, (nmeaGAGSV *)node->pack))
                {
                    free(node);
                    node = 0;
                }
                break;
								case GQGSV:
                if(0 == (node->pack = malloc(sizeof(nmeaGQGSV))))
                    goto mem_fail;
                node->packType = GQGSV;
								//FIX-NOW
                if(!nmea_parse_GQGSV(
                    (const char *)parser->buffer + nparsed,
                    sen_sz, (nmeaGQGSV *)node->pack))
                {
                    free(node);
                    node = 0;
                }
                break;
								/* add KSXT */
								case KSXT:
                if(0 == (node->pack = malloc(sizeof(nmeaKSXT))))
                    goto mem_fail;
                node->packType = KSXT;
								//FIX-NOW
                if(!nmea_parse_KSXT(
                    (const char *)parser->buffer + nparsed,
                    sen_sz, (nmeaKSXT *)node->pack))
                {
                    free(node);
                    node = 0;
                }
                break;
								case PSRDOPA:
                if(0 == (node->pack = malloc(sizeof(nmeaPSRDOPA))))
                    goto mem_fail;
                node->packType = PSRDOPA;
								//FIX-NOW
                if(!nmea_parse_PSRDOPA(
                    (const char *)parser->buffer + nparsed,
                    sen_sz, (nmeaPSRDOPA *)node->pack))
                {
                    free(node);
                    node = 0;
                }
...
/**
 * \brief Withdraw top packet from parser   //从解析器中取出顶部数据包
 * @return Received packet type
 * @see nmeaPACKTYPE
 */
int nmea_parser_pop(nmeaPARSER *parser, void **pack_ptr)
{
    uint8_t retval = GPNON;
    nmeaParserNODE *node = (nmeaParserNODE *)parser->top_node;
