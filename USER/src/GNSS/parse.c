...
//    NMEA_ASSERT(buff);
    ...
    if(buff_sz < 5)
        ...
        return GQGSV;
		else if(0 == memcmp(buff, pheads[8], 4))
        return KSXT;
		else if(0 == memcmp(buff, pheads[9], 7))
        return PSRDOPA;
...
//    NMEA_ASSERT(buff && res_crc);
    ...
    *res_crc = -1;
    ...
    for(;buff < end_buff; ++buff, ++nread)
    {
        ...
        {
            if((buff + 11 <= end_buff && '\r' == buff[9] && '\n' == buff[10]))
            {
                *res_crc = nmea_atoi(buff + 1, 8, 64);
                nread = buff_sz - (int)(end_buff - (buff + 11));
								*res_crc=1;

						}
						else if(buff + tail_sz <= end_buff && '\r' == buff[3] && '\n' == buff[4])
            {
...
//    NMEA_ASSERT(buff && pack);
    ...
    memset(pack, 0, sizeof(nmeaGLGSV));
...
//    NMEA_ASSERT(buff && pack);
    ...
    memset(pack, 0, sizeof(nmeaGAGSV));
...
//    NMEA_ASSERT(buff && pack);
    ...
    memset(pack, 0, sizeof(nmeaGQGSV));
...
/*add-funciton-KSXT-具体参数向上参考*/
int nmea_parse_KSXT(const char *buff, int buff_sz, nmeaKSXT *pack)
{
    char time_buff[NMEA_TIMEPARSE_BUF];
//    NMEA_ASSERT(buff && pack);

    memset(pack, 0, sizeof(nmeaKSXT));

//    nmea_trace_buff(buff, buff_sz);

    if(20 != nmea_scanf(buff, buff_sz,
        "$KSXT,%s,%f,%f,%f,%f,%f,%f,%f,%f,%d,%d,%d,%d,%f,%f,%f,%f,%f,%f,%f,*",
        &(time_buff[0]), &(pack->lon), &(pack->lat),&(pack->Height), 
				&(pack->Heading), &(pack->pitch), &(pack->track_true), &(pack->Vel),
        &(pack->Roll), &(pack->POS_qual), &(pack->heading_qual), &(pack->solnSVs),
        &(pack->solnSVs_s), &(pack->east), &(pack->north), &(pack->up),
        &(pack->EastVel), &(pack->NorthVel), &(pack->UpVel), &(pack->Nones)))
		{
				nmea_error("KSXT parse error!");
				free(pack);
        return 0;
		}
...
//    NMEA_ASSERT(buff && pack);
		...
		int nsen;
    memset(pack, 0, sizeof(nmeaPSRDOPA));

    nmea_trace_buff(buff, buff_sz);

//        "#PSRDOPA,,,,,,,,,;%f,%f,%f,%f,%f,%f,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d*",
//               "#PSRDOPA,%s,%d,%f,%s,%d,%f,%d,%d,%d;%f,%f,%f,%f,%f,%f,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d*",
        nsen=nmea_scanf(buff, buff_sz,
        "#PSRDOPA,%s,%d,%f,%s,%d,%f,%d,%d,%d;%f,%f,%f,%f,%f,%f,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d*",
        &(pack->utcs[0]), &(pack->ones), &(pack->twos),&(pack->dssf[0]), 
				&(pack->threes), &(pack->fours), &(pack->fives), &(pack->sixs),
        &(pack->sevens), &(pack->GDOP), &(pack->PDOP), &(pack->HDOP),
        &(pack->HTDOP), &(pack->TDOP),&(pack->eights),&(pack->nines),
				&(pack->tens),&(pack->ele),&(pack->twices),&(pack->threten),
				&(pack->fourten),&(pack->fiveten),&(pack->sixten),&(pack->sevten),&(pack->ten_ten),&(pack->twice_ten));	
    if(nsen != 26 && nsen != 25 && nsen != 24)
    {
        nmea_error("PSRDOPA parse error!");
				free(pack);
        return 0;
    }
...
/**
 * \brief Fill nmeaINFO structure by GGA packet data.  //用GGA数据包填充nmeaINFO结构。
...
{
	info->EastVel=pack->EastVel;
	info->NorthVel=pack->NorthVel;
	info->UpVel=pack->UpVel;
//	info->lat=pack->lat;
//	info->lon=pack->lon;
