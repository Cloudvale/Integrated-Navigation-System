...
//            printf("%s\r\n", fattester.fileinfo.fname); /* 打印文件名 */
						res=memcmp(fattester.fileinfo.fname,"0:/GNSS-IMU.txt",15);
...
uint8_t mf_fmkfs(uint8_t *path, uint8_t opt, uint16_t au)
{
    ...
    return f_mkfs((const TCHAR *)path, &temp, 0, _MAX_SS);    /* 格式化,默认参数,workbuf,最少_MAX_SS大小 */
}
...
/* use-for */
//uint8_t use_mf_scan_files(uint8_t * path)
//{
//	 FRESULT res;
//	 int scan_ok=0;
//	
//    res = f_opendir(&use_fattester.dir, (const TCHAR *)path); /* 打开一个目录 */

//    if (res == FR_OK)
//    {
//        while (1)
//        {
//            res = f_readdir(&use_fattester.dir, &use_fattester.fileinfo);   /* 读取目录下的一个文件 */
//					
//					

//            if (res != FR_OK || use_fattester.fileinfo.fname[0] == 0)
//            {
//                break;  /* 错误了/到末尾了,退出 */
//            }
//						scan_ok++;
//            
////						scan_ok=memcmp(use_fattester.fileinfo.fname,filename,sizeof(&filename));
////						if(scan_ok==0)                                                 /* 当scan_ok为0时，则为扫描到想要文件 */
////						{
////							return scan_ok;
////						}
////						else
